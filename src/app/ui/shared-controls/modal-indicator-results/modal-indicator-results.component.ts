...
@Component({
    selector: 'app-modal-indicator-results',
    templateUrl: './modal-indicator-results.component.html',
    styleUrls: ['./modal-indicator-results.component.scss'],
})
export class ModalIndicatorResultsComponent implements OnInit {
    @Input() metasIndicadores: any;
    configGeneralInfo: ConfiguracionGeneral;
    dataRow: any;
    metasIndicadoresCapturaDTO: ResultadoIndicadorCapturaDto[];
    form: FormGroup;
    ...
    constructor(
        ...
        public dialogRef: MatDialogRef<ModalIndicatorResultsComponent>,
        private metasAreaResponsableApi: ResultadosIndicadorService,
    ...
    onViewMi(event: any) {
        ...
        var metaCapturada: ResultadoIndicadorCapturaDto = JSON.parse(JSON.stringify(rowData.value));
    ...
    obtenerListaElementos() {
        this.metasAreaResponsableApi
            ...
            .subscribe((response: ResponseV1<any>) => {
                ...
                if (response) {
                    this.metasIndicadoresCapturaDTO = response.output;
                    let arrFormGroup = this.metasIndicadoresCapturaDTO.map(ResultadoIndicadorCapturaDto.asFormGroup);
    ...
    onUpdateConfiguration() {
        ...
        arrCaptura.elementos.forEach((item: any) => {
            if (!item.noAplicaResultados) {
                item.justificacionNaresultados = '';
            }
            else {
                item.resultadoActualResultados = null;
            }
            item.usuarioModificacion = usrModificacion;

            if (item.noAplicaResultados && item.justificacionNaresultados?.length == 0) {
                Alert.warn('', `Al seleccionar la opción Check de NA, se debe de capturar una justificación`);
                continuaGuardado = false;
            }

            if (item.cumplimiento == 'NO' && item.justificacion?.length == 0) {
                Alert.warn('', `Cuando no se logró el cumplimiento, debe definir una justificación`);
                continuaGuardado = false;
            }
            else {
                if (item.cumplimiento == 'SI') {
        ...
        this.metasAreaResponsableApi
            .addMetasAreaResponsable(arrCaptura.elementos)
            .subscribe((response: ResponseV1<any>) => {
                if (response.exito) {
    ...
    changeValueResultadoActual(event: any, row: any) {
        if (event.target.value >= (row.controls.metaActual.value ?? 0)) {
            row.controls.cumplimiento.setValue('SI')
        } else {
            row.controls.cumplimiento.setValue('NO')
        }
    }

    onCheckedNa(event: any, row: any) {
        if (event.checked) {
            row.controls.cumplimiento.setValue('NA')
        }
        else {
            row.controls.cumplimiento.setValue('NO');
        }
