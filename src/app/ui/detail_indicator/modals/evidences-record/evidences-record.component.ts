...
import { EvidenceDTO, TablePaginatorSearch } from 'src/app/utils/models';
...
export class EvidencesRecordComponent implements OnInit {
  ...
  public getEvidences(filters: TablePaginatorSearch): void {
    ...
    this.apiEvidencesCatalogService.getAllEvidence(filters).subscribe((response) => {
      let dataTodasLasEvidencias = response.output.map((detailsIndicator) => new EvidenceDTO().deserialize(detailsIndicator));
  ...
  onChange($event: any, indica: DetailsIndicatorAllEvidencesDTOV1) {
    // console.log(indica);
    const newIndicador: DetailsIndicatorEvidenciaDTOV1 = new DetailsIndicatorEvidenciaDTOV1();
    ...
    this.evidencias = res; //console.log(this.evidencias);
  }
  ...
  public guardar(): void {
    ...
    this.detailsIndicator.getAddEvidencias(this.evidencias).subscribe((response) => {
