...
export class RubricaIndicatorsControlComponent implements OnInit {
  ...
  public getRubricas(): void {
    ...
    filters.filter = {"indicadorId":indicadorId}
    ...
    let ochoList ;
    let diezList ;
    this.detailsIndicator.getAllRubricaIndicadores(filters).subscribe((response) => {
      this.data = response.output.map((detailsIndicator) => new MisionDTO().deserialize(detailsIndicator)); console.log(this.data);        
  ...
  deleteRubrica(indicador: RubricasDTOV1): void {//console.log(indicador);
    Alert.confirm('Eliminar Rúbrica', `¿Deseas eliminar la Rúbrica?`).subscribe((result) => {
      if (!result || !result.isConfirmed) {
        return;
      };
      this.detailsIndicator.deleteRubrica(indicador).subscribe(() => {
        this.getRubricas();
  ...
  editRubrica(rubricaDatos: RubricasDTOV1): void {
    this.rubricaRecords
        .open({ dataSave: rubricaDatos })
        .afterClosed()
        .subscribe(() => this.getRubricas());
  }

  public addRubrica(): void {
    this.rubricaRecords
        .open()
        .afterClosed()
        .subscribe(() => this.getRubricas());
