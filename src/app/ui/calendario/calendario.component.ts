...
import { DataService } from './calendario.service';
...
@Component({
  ...
  standalone: true,
  imports: [ ApplyPipe, DxSchedulerModule, DxTemplateModule, CommonModule, SharedModule],
...
export default class CalendarioComponent {
  ...
  ariaDescription = () => {
    ...
    return (disabledDates?.length === 1) ? `${disabledDates} is a disabled date` : `${disabledDates.join(', ')} are disabled dates`;
  };
  ...
  setComponentAria(element: any): void {
...
@NgModule({
  ...
  declarations: [CalendarioComponent, ApplyPipe],
