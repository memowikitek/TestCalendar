...
export class IndicatorsRecordComponent implements OnInit {
  ...
  private configureForm(){
    this.indicadorRecordForm = this.formBuilder.group({
      ...
      metaInstitucional: [null, [Validators.required]],
  ...
  submit(): void {
    ...
    if (this.data.procesoEvaluacionId) {
      ...
      indicadores.metaInstitucional = tmp.metaInstitucional;
  ...
  selectionComponente(id: number) {
    ...
    const filtrados = evalList.filter((row: { componenteId: number; }) => row.componenteId === id);
