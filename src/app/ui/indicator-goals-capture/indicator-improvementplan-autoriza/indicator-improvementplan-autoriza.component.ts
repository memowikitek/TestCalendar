...
@Component({
  selector: 'app-indicator-improvementplan-autoriza',
  templateUrl: './indicator-improvementplan-autoriza.component.html',
  styleUrls: ['./indicator-improvementplan-autoriza.component.scss']
})
export class IndicatorImprovementplanAutorizaComponent {
  ...
  numeroEtapa = 6; // sirve para referenciar la estaba en la pantalla de detalles
  ...
  ngOnInit(): void {
    ...
    this.indicatorGoalCaptureDataService.getCeIndicadoresEtapa6PMDData().subscribe(resp => {
  ...
  private getAllCicloEvaIndicadores(filter: TablePaginatorSearch) {
    ...
    this.data = [];
    this.EvaCService.getCeIndicadoresEtapa6PMD(filter).subscribe((response) => {
  ...
  captureData(row: any) {
    row.esAreaConsolidada = false;
    if(this.dataCaptura && this.dataCaptura.esAreaConsolidada)
    {
      row.esAreaConsolidada = this.dataCaptura.esAreaConsolidada
    }
