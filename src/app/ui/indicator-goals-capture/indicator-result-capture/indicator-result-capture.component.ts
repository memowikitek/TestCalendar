...
@Component({
  selector: 'app-indicator-result-capture',
  templateUrl: './indicator-result-capture.component.html',
  styleUrls: ['./indicator-result-capture.component.scss']
})
export class IndicatorResultCaptureComponent {
  ...
  numeroEtapa = 3; // sirve para referenciar la estaba en la pantalla de detalles
  ...
  captureData(row: any) {
    ...
    if (row.esAreaConsolidada)
      {
        Alert.confirmAreaConsolidada('Área consolidada','Tu dependencia de área realizará la captura de resultados. Puedes conocer tus indicadores y consultar tus datos.')
        .subscribe(resp =>
        {
    ...
    if (this.esAutorizadorNa && this.esUsuarioACentral) {
      this.dialog.open<ModalAutorizeNaResultComponent>(ModalAutorizeNaResultComponent, {
        panelClass: '',
        data: row || null,
        width: '70vw',
        minWidth: '70vw',
