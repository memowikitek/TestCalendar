...
@Component({
  selector: 'app-indicator-improvementplan-execution',
  templateUrl: './indicator-improvementplan-execution.component.html',
  styleUrls: ['./indicator-improvementplan-execution.component.scss']
})
export class IndicatorImprovementplanExecutionComponent {
  ...
  columns = ['nombreCampus', 'area', 'indicadorTipoResultado', 'nivelModalidad', 'claveNombreElemento', 'nombreIndicador', 'select', 'edit','estatusRI'];
  ...
  private getAllCicloEvaIndicadores(filter: TablePaginatorSearch) {
    ...
    this.data = [];
    this.EvaCService.getCeIndicadoresEtapa6ExecutionRI(filter).subscribe((response) => {
  ...
  viewData(row: any) 
  {
    ...
    row.campusId = row.campusId;
    row.AreaResponsableid = row.areaId ? row.areaId : row.areaResponsableId;
