...
@Component({
  selector: 'app-indicator-evidence-capture',
  templateUrl: './indicator-evidence-capture.component.html',
  styleUrls: ['./indicator-evidence-capture.component.scss']
})
export class IndicatorEvidenceCaptureComponent {
    ...
    this.EvaCService.getCeIndicadoresEtapa2(filter).subscribe((response) => {
      if (response.output) {
        ...
        if (arrAreaCentral.length > 0) {
          ...
          this.columns = (this.revIns) ? ['nombreCampus', 'area', 'indicadorTipoResultado', 'nivelModalidad', 'claveNombreElemento', 'nombreIndicador', 'subAreaCentral','claveEvidencia', 'estatusDescripcion', 'edit', 'estatusRI', 'usuarioRevisor'] : ['nombreCampus', 'area', 'indicadorTipoResultado', 'nivelModalidad', 'claveNombreElemento', 'nombreIndicador', 'subAreaCentral','claveEvidencia', 'estatusDescripcion', 'edit'];
        }
  ...
  captureData(row: any) {
    ...
    if (this.esAutorizadorNa && this.esUsuarioACentral) {
      this.dialog.open<ModalAutorizeNaEvidenceComponent>(ModalAutorizeNaEvidenceComponent, {
        panelClass: '',
        data: row || null,
        width: '70vw',
        minWidth: '70vw',
  ...
  obtenerTextoEtapa(){
    return  `${this.cicloEva.cicloEvaluacion} | Etapa 2`  
  }
