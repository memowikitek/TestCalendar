...
@Component({
  selector: 'app-indicator-autoevaluationreview-campusdeparea',
  templateUrl: './indicator-autoevaluationreview-campusdeparea.component.html',
  styleUrls: ['./indicator-autoevaluationreview-campusdeparea.component.scss']
})
export class IndicatorAutoevaluationreviewCampusdepareaComponent {
  ...
  private getAllCicloEvaIndicadores(filter: TablePaginatorSearch) {
    ...
    this.EvaCService.getCeIndicadoresEtapa5DepAreaVw1(filter).subscribe((response) => {
      if (response.output) {
        console.log("etapa 5 deparea", response.output);
        ...
        this.msjPositivo = response.mensaje; console.warn('"'+this.msjPositivo+'"');
      }
    });
  }
  ...
  activaRetroalimentacion(event: any) {
    this.EvaCService.addUpdateCeEtapa5RevisionAutoevaluacionDepAreaSesion(event, this.users.userSession.id).subscribe(resp => {
      if (resp.exito) {
        let data ={cicloEvaluacionId : event.cicloEvaluacionId , campusId : event.campusId , areaId : event.areaId, campus: event.campus,
          areaResponsable:event.areaResponsable,sesionRetro : event.sesionRetro}  
          this.dataService.setAutoEvaluationReviewData(data,true);
          this.router.navigate(['/indicator-goals-capture/autoevaluation-review-capture-campusdeparea-detail'])
      }
    });
  }

  seleccionaRow(event: any) {
    let data ={cicloEvaluacionId : event.cicloEvaluacionId , campusId : event.campusId , areaId : event.areaId, campus: event.campus,
      areaResponsable:event.areaResponsable,sesionRetro : event.sesionRetro};
    this.dataService.setAutoEvaluationReviewData(data,true);
    this.router.navigate(['/indicator-goals-capture/autoevaluation-review-capture-campusdeparea-detail'])
  ...
  guardaComentario(event: any) {
    ...
    this.EvaCService.addUpdateCeEtapa5RevisionAutoevaluacionDepAreaSesion(event, this.users.userSession.id).subscribe(resp => {
      if (resp.exito) {
        this.basicNotification.notif("success", 'Comentario de retroalimentación actualizado correctamente.', 2000);
      }
    });
  }
