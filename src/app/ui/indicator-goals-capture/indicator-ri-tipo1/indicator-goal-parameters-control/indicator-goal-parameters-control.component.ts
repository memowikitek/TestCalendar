...
export class IndicatorGoalParametersControlComponent implements OnInit {
  ...
  obtenerDatosDeStorage(): any {
    let datoStorage = JSON.parse(localStorage.getItem('CE'));
    if (datoStorage[0]) {
  ...
  aplicarFiltro() {
    this.etapas = this.paramsAll(this.cicloFilterForm.get('etapas').value);
    ...
    const Etapas = (this.etapas) ? (this.etapas.length >= 1 && this.etapas.length <= this.etapasList.length) ? this.etapas : null : null;
  ...
  CleanFilters(c: any) {
    ...
    if (c == 'all') {
      this.cicloFilterForm.get('etapas').setValue(null);
  ...
  getEstatus(arr: any) {
    ...
    switch (etapaId) {
      ...
      case 6:
        ...
        break;
      default:
        console.warn('No localizo id etapa en control paramentros');
