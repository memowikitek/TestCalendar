...
import { EstatusEtapa4Autoevaluacion } from 'src/app/utils/models/enums-estatus-etapas';
import { environment } from 'src/environments/environment';


@Component({
  selector: 'app-indicator-autoevaluation-capture-by-level',
  templateUrl: './indicator-autoevaluation-capture-by-level.component.html',
  styleUrls: ['./indicator-autoevaluation-capture-by-level.component.scss']
})
export class IndicatorAutoevaluationCaptureByLevelComponent {
  ...
  private GetCeEvaluacionEtapaAll(filter: TablePaginatorSearch) {
    ...
    if (!this.visualizarInformacionEtapa()){ // si no es etapado no llama a la api
      console.log('No consultará etapa4');
      return; 
    }
    this.EvaCService.getCeEvaluacionEtapa4All(filter, this.indicadorCapura.CicloEvaluacionInfo.cicloEvaluacionId).subscribe((response) => {
      if (response.output) {
  ...
  captureData(row: any) {
    ...
    this.dialog.open<ModalAutoevaluationCaptureComponent>(ModalAutoevaluationCaptureComponent, {
      panelClass: '',
      data: row || null,
      width: '70vw',
      minWidth: '70vw',
  ...
  puedeVisualizarEditar(element: any) {
    ...
    if (this.soloVisualiza || element.estatusEtapa == EstatusEtapa4Autoevaluacion.naautorizado) {
      return false;
    }
  ...
  { 
    ...
    if ((element.estatusEtapa != EstatusEtapa4Autoevaluacion.naautorizado) ) {
      return false;
    }
