...
@Component({
  selector: 'app-modal-autorize-na-evidence',
  templateUrl: './modal-autorize-na-evidence.component.html',
  styleUrls: ['./modal-autorize-na-evidence.component.scss']
})
export class ModalAutorizeNaEvidenceComponent implements OnInit, AfterViewInit {
  addOnBlur = true;
  ...
  public readonly evidenciaData: any,
    ...
    if (evidenciaData.autorizaNa != null) {
      this.selectedValueAutorize = evidenciaData.autorizaNa == true ? 7 : 6;
    }
  }
  ...
  guardar() {
    ...
    if (this.selectedValueAutorize == 6) {// No autorizado requiere comentario obligatorio
      this.evidenciaData.estatusEtapa =6;
      this.evidenciaData.autorizaNa = false;
      if (!this.evidenciaData.comentAutorizaNa) {
        this.errorAutorizado = true;
        return;
      }
    }
    else {
      if (this.selectedValueAutorize == 7) { // autorizado no requiere comentarios
        this.evidenciaData.estatusEtapa = 7;
    ...
    this.EvaCService.addUpdateCeEtapa2Meta(this.evidenciaData).subscribe((response) => {
      if (response.output) {
        ...
        this.ref.close(true);
        let msgNa = this.evidenciaData.estatusEtapa ==  6 ? `N/A no autorizado` : this.evidenciaData.estatusEtapa ==  7 ? `N/A autorizado` : `Evidencia guardada`;
