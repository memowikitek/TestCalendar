...
@Component({
  selector: 'app-modal-autorize-na-improventplan',
  templateUrl: './modal-autorize-na-improventplan.component.html',
  styleUrls: ['./modal-autorize-na-improventplan.component.scss']
})
export class ModalAutorizeNaImproventplanComponent {
// #region variables generales de modal captura

  //titulo del modal
  title = '';
  NombreEtapa = 'Autorizar NR Plan mejora - Diseño'
  ...
  esAutorizadorNR = true
  errorJustificacionNr = false
  errorJustificacionNrComment = false
  ...
  guardaDecisionNOautorizadorNR(){
    this.errorJustificacionNrComment = false
  ...
  guardaDecisionAUTORIZADORNR(){
    ...
    else
    {
      if (this.modalData.estatusEtapa == this.estatusEtapa6Plandemejora.noSeRealiza_No_Autorizado){
        ...
        if(!this.modalData.comentAutorizaNr)
          {
          this.errorJustificacionNrComment = true;
          return;
          }
      }
    }
    ...
    this.EvaCService.updateCEEtapa6PlanMejoraDecision(this.modalData).subscribe(response => {
      if (response.output) {
        ...
        if (this.modalData.estatusEtapa == this.estatusEtapa6Plandemejora.noSeRealiza_Autorizado)
        {
          this.basicNotification.notif("success", `N/R autorizado correctamente` );
        }
        else
        {
          if (this.modalData.estatusEtapa == this.estatusEtapa6Plandemejora.noSeRealiza_No_Autorizado)
          {
              this.basicNotification.notif("success", `N/R no autorizado correctamente` );
          }
        }
      }
      else {
  ...
  configuraVista() {
    ...
    this.title = (this.modalData.esVista) ? `${this.NombreEtapa}` : `${this.NombreEtapa}`;  
